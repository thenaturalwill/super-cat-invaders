...
function CreateGameElement(width, height, xPos, yPos, speed, img, TTL) {
  return {
    width: width,
    height: height,
    xPos: xPos,
...
// Create the Ship
const SHIP = CreateGameElement(
  SHIP_SIZE.width,
  SHIP_SIZE.height,
  (C_WIDTH - SHIP_SIZE.width) / 2,
  C_HEIGHT - SHIP_SIZE.height - 5,
  2.5,
  SHIP_IMG,
  0
);
...
let enemyMissile = {
  ...
  fire: function (yPos, xPos, width) {
    ...
    enemyMissilesOnScreen.push(
      CreateGameElement(
        MISSILE_SIZE.width,
        MISSILE_SIZE.height + 5,
        this.xPos,
        this.yPos,
        this.speed,
        E_MISSILE_IMG,
    ...
    for (let i = 0; i < enemiesOnScreen.length; i++) {
      if (enemiesOnScreen.length <= 12) {
        if (i === Math.floor(Math.random() * 300 + 1)) {
          enemyMissile.fire(
            enemiesOnScreen[i].yPos,
            enemiesOnScreen[i].xPos,
            enemiesOnScreen[i].width
          );
        }
...
function enemyMissileCollision() {
  for (let i = 0; i < enemyMissilesOnScreen.length; i++) {
    try {
      ...
      if (
        enemyMissilesOnScreen[i].yPos >= SHIP.yPos &&
      ...
      for (let x = 0; x < buildings.length; x++) {
        if (
          enemyMissilesOnScreen[i].yPos >= buildings[x].yPos &&
          enemyMissilesOnScreen[i].xPos >= buildings[x].xPos &&
          enemyMissilesOnScreen[i].xPos <=
            buildings[x].xPos + buildings[x].width
        ) {
          enemyMissilesOnScreen.splice(i, 1);
  ...
  for (let i = 0; i < playerMissilesOnScreen.length; i++) {
    try {
      ...
      if (score >= 35000) {
        ...
        playerMissilesOnScreen[i].TTL -= playerMissilesOnScreen[i].speed + 4;
      } else if (score >= 70000) {
        playerMissilesOnScreen[i].yPos -= playerMissilesOnScreen[i].speed + 8;
        playerMissilesOnScreen[i].TTL -= playerMissilesOnScreen[i].speed + 8;
      } else {
        playerMissilesOnScreen[i].yPos -= playerMissilesOnScreen[i].speed;
        playerMissilesOnScreen[i].TTL -= playerMissilesOnScreen[i].speed;
      }
      if (DEBUG === true) {
        console.log(playerMissilesOnScreen[i].TTL);
      }
    } catch (error) {
      console.error(error);
    }
...
function playerMissileCollision() {
  for (let i = 0; i < playerMissilesOnScreen.length; i++) {
    try {
      ...
      for (let j = 0; j < enemiesOnScreen.length; j++) {
        if (
          playerMissilesOnScreen[i].yPos - enemiesOnScreen[j].height <=
            enemiesOnScreen[j].yPos &&
          playerMissilesOnScreen[i].TTL >= enemiesOnScreen[j].yPos &&
          playerMissilesOnScreen[i].xPos >= enemiesOnScreen[j].xPos &&
          playerMissilesOnScreen[i].xPos <=
            enemiesOnScreen[j].xPos + enemiesOnScreen[j].width
        ) {
      ...
      for (let x = 0; x < buildings.length; x++) {
        if (
          playerMissilesOnScreen[i].yPos <= buildings[x].yPos &&
